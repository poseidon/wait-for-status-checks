...
async function run(): Promise<void> {
  try {
    ...
    const successConclusions = (
      core.getInput('success_conclusions') || 'success,skipped'
    )
      .split(',')
    ...
    const invalidConclusions = successConclusions.filter(
      conclusion => !validConclusions.includes(conclusion)
    )
    if (invalidConclusions.length > 0) {
      throw new Error(
        `Invalid success_conclusions: ${invalidConclusions.join(', ')}. Valid values: ${validConclusions.join(', ')}`
      )
    }
    ...
    if (!validTimeoutBehaviors.includes(timeoutBehaviorInput)) {
      throw new Error(
        `Invalid timeout_behavior: ${timeoutBehaviorInput}. Valid values: ${validTimeoutBehaviors.join(', ')}`
      )
    }
